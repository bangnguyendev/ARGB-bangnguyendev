...
#define WLED_CODENAME "bangnguyendev"
...
WLED_GLOBAL char ntpServerName[33] _INIT("vn.pool.ntp.org");   // NTP server to use
...
#ifndef SERVERNAME
WLED_GLOBAL char serverDescription[33] _INIT("ARGB-bangnguyendev");  // Name of module - use default
#else
...
WLED_GLOBAL char mqttDeviceTopic[MQTT_MAX_TOPIC_LEN+1] _INIT("argb");         // main MQTT topic (individual per device, default is wled/mac)
WLED_GLOBAL char mqttGroupTopic[MQTT_MAX_TOPIC_LEN+1]  _INIT("argb/all"); // second MQTT topic (for example to group devices)
...
WLED_GLOBAL char mqttClientID[41] _INIT("ARGB");               // override the client ID
